window.addEventListener('DOMContentLoaded', () => {
  ...
  function loadGameStats() {
    try {
      ...
      const pinballData = JSON.parse(localStorage.getItem('pinball_game_data') || '{}');
      ...
      document.getElementById('pinball-level').textContent = pinballData.level || '1';
      document.getElementById('pinball-best').textContent = pinballData.bestScore || '0';
    } catch (e) {
  ...
  function handleGameData() {
    ...
    if (window.location.hash) {
      try {
        ...
        if (data.game && data.score !== undefined) {
